...
namespace sdk {
	...
	void entity_t::get_name(std::string& out) {
		...
		std::uintptr_t client_state = m_memory->read< std::uintptr_t >(m_modules->engine_dll + 0x588FEC); //m_dwClientState
		std::uintptr_t user_info_table = m_memory->read< std::uintptr_t >(client_state + 0x52C0); //m_dwClientState_PlayerInfo
